...
export const registerStudent = async (req: Request, res: Response) => {
  try {
    ...
    if (!isValidRegistration(reg_no)) {
      return res
        .status(400)
        .json({ status: `error`, message: `Invalid Registration Number` });
    }
    ...
    if (checkStudent)
      return res
        ...
        .json({
          status: `error`,
          message: `Student ${reg_no} already registered, please login`,
        });
    ...
    if (confirmStudent === "not found")
      return res
        ...
        .json({
          status: `error`,
          message: `${reg_no} does not exist, contact the Student Affairs Unit`,
        });
    ...
    if (!findStudent)
      return res
        .status(400)
        .json({ status: `error`, message: `Unable to create` });
    ...
    return res
      ...
      .json({
        status: `success`,
        data: findStudent,
        message: `Enrolment Successful, Your login details have been sent to your mail: ${studentData.email}`,
      });
  } catch (error: any) {
